...
while($row = db_fetch_assoc($milestones)){
foreach($row as $val){
$template_names[] = $val;
}
}
...
$newcol = $r['milestone_display_name'];
  ...
  $mstone .= '<tr><td><label class="inner-format" for="milestones"><span>'.$newcol.'</span></label></td>
  <td><input type="text" class="table-size" id="templateDate'.$line.'" name="tempDate'.$line.'" readonly/></td>
<td><input type="text" class="table-size" id="templateDuration'.$line.'" name="hithere'.$line.'" value="'.$dura[$line].'" readonly/></td>
 <td><input type="text" class="table-size" id="actualDate'.$line.'" name="'.strtolower($key).'_date" readonly/></td>
 <td><input type="text" class="table-size" id="actualDuration'.$line.'" name="'.strtolower($key).'" value="'.$dura[$line].'" /></td></tr>';
$i--;
}
